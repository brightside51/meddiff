...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_start, t, cond=None, noise=None, **kwargs):
        ...
        return {
            ...
            #'l1_loss': F.l1_loss(x_start, x_recon),
            #'mse_loss': F.mse_loss(x_start, x_recon),
            'x_start': x_start,
            #'x_noisy': x_noisy,
            'x_recon': x_recon
            }
...
class Trainer(object):
    ...
    def train(
        ...
        while self.step < self.train_num_steps:
            ...
            #if self.step < 30000 or (self.step >= 30000 and self.step % 10000 < 9000):
            if (self.step // 1000) % 2 == 0:
            #if self.step % 10000 < 9000:
                wave = True
            else: wave = False
            wave = False
            ...
            for i in range(self.gradient_accumulate_every):
                ...
                with autocast(enabled=self.amp):
                    ...
                    if wave: self.scaler.scale(loss / self.gradient_accumulate_every).backward()
                    else:
                        
                        """
                        img_start = (((losses['x_start'] + 1.0) / 2.0) * (self.ema_model.vqgan.codebook.embeddings.max() -
                                    self.ema_model.vqgan.codebook.embeddings.min())) + self.ema_model.vqgan.codebook.embeddings.min()
                        img_start = self.ema_model.vqgan.decode(img_start, quantize=True)
                        img_recon = (((losses['x_recon'] + 1.0) / 2.0) * (self.model.vqgan.codebook.embeddings.max() -
                                    self.ema_model.vqgan.codebook.embeddings.min())) + self.ema_model.vqgan.codebook.embeddings.min()
                        img_recon = self.ema_model.vqgan.decode(img_recon, quantize=True)"""

                        img_recon1 = list(map(lambda n: self.model.sample(batch_size=1), num_to_groups(1, 1)))[0]
                        img_recon = img_recon1.clone(); del img_recon1
                        recon_loss = F.mse_loss(img_recon, data[0].unsqueeze(0))
                        ssim_loss = SSIM(img_recon, data[0].unsqueeze(0))
                        msssim_loss = MS_SSIM(img_recon, data[0].unsqueeze(0))
                        logits_fake, pred_fake = self.model.vqgan.video_discriminator(img_recon)
                        ...
                        #self.scaler.scale(1 - ssim_loss).backward()
            ...
            if not wave:
                #self.train_logger.experiment.add_image("GT Image", data[0, :, 15], self.step)
                ...
                self.train_logger.experiment.add_scalar("Train/Disc Loss", disc_loss, self.step)
                self.train_logger.experiment.add_scalar("Train/SSIM Index", ssim_loss, self.step)
                self.train_logger.experiment.add_scalar("Train/MS-SSIM Index", msssim_loss, self.step)
