...
conda init --all
conda activate meddiff1

#python -m pip install -r /nas-ctm01/homes/pfsousa/meddiff/requirements.txt
#nvidia-smi
#pip install torch==2.5.1+cu118 torchvision==0.20.1+cu118 torchaudio==2.5.1+cu118 --index-url https://download.pytorch.org/whl/cu118 --no-cache-dir --no-deps
#python -c "import torch; print(torch.__version__); print(torch.cuda.is_available()); print(torch.cuda.get_arch_list())"

/nas-ctm01/homes/pfsousa/.conda/envs/meddiff/bin/python /nas-ctm01/homes/pfsousa/meddiff/runs/main_V0.py
#python main_V0.py
