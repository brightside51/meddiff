...
from infer_ddpm import infer_ddpm

# # ============================================================================================

# Argument Initialisation
luna25_args = data_parser(dataset = 'luna25_nodule', dataV = 'V0', save = True)
run_args = run_parser(model = 'meddiff', runV = 'Vluna', save = True)

# WandB Setup
wandb.login()
run_logger = wandb.init(entity = "brightside51", project = run_args.model,
                        name = f"{run_args.runV} ({datetime.now().strftime('%H:%M %d/%m/%Y')})",
                        config = {"dataV": luna25_args.dataV, "runV": run_args.runV,})
